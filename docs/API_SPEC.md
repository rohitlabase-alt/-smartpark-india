...
| POST | /parking-sessions/{id}/cancel | operator | force-exit: ACTIVE → CANCELLED, release slot, cancel reservation (role-only, VERIFIED operator of the facility) |
...
> **Implementation status (Phase 9, Block 1 — entry/exit foundation, Block 2 — session read/UI, Block 3 — pass/gate tokens + occupancy, Block 4 — lifecycle hardening + operator force-exit):** all endpoints are **implemented** (mounted at `/api/v1/parking-sessions`, authenticated, authorization enforced server-side — the reservation owner OR the VERIFIED operator who owns the facility; any other caller gets `404`, no existence disclosure). The one exception is intentionally **operator-only**: `POST /parking-sessions/{id}/cancel` (Block 4.2) requires the `PARKING_OPERATOR` role — a reservation owner may never cancel their own session (use `exit` instead).
>
...
> - **Block 3 — occupancy integrity:** slot status changes away from `OCCUPIED` throw `409 SLOT_IN_USE` **only** when an active session still references the slot (row-lock serialization via `findByIdForUpdate`); genuine manual corrections of stuck slots remain allowed. Entry/exit still emit the same transaction audits as Block 1 — `PARKING_SESSION_ENTRY` / `PARKING_SESSION_EXIT` (Block 3 added no new success verbs). Rejections write `GATE_ENTRY_REJECTED` / `GATE_EXIT_REJECTED` (exit `SESSION_NOT_ACTIVE` only) with `metadata.reason` = the error code — tokens never appear in audit metadata. `GATE_ENTRY_VERIFIED` / `GATE_EXIT_VERIFIED` / `SLOT_OCCUPIED` / `SLOT_RELEASED` remain **reserved AUDIT_ACTIONS vocabulary** in the shared schema but are **not raised by any current flow** (docs corrected in Session 12).
> - **Block 4.2:** `POST /parking-sessions/{id}/cancel` → `200 { session }` (session `ACTIVE → CANCELLED`). **Operator-only**: `requireRole("PARKING_OPERATOR")` + the service requires the org to be **VERIFIED** (`403 OPERATOR_NOT_VERIFIED`) and to own the session's facility (derived server-side — the body never carries a `facilityId`). An operator of another facility or the reservation owner gets `404 SESSION_NOT_FOUND` / `403`, no existence disclosure. Body `{ reason?: string }` (optional, ≤500 chars, `.strict()`) is stored on the reservation (`cancel_reason`/`cancelled_at`). Atomic: session → `CANCELLED` (`exit_at` stamped), slot → `AVAILABLE`, reservation `ACTIVE → CANCELLED`, availability cache mirrored, `PARKING_SESSION_CANCELLED` audit record written with metadata `{ facilityId, slotId, reservationId, cancelledBy: "OPERATOR", reason?, slotReleased }` (never token material). Guarded/row-locked transition (`FOR UPDATE` session → reservation → slot) means exactly one of a normal exit and a force-cancel can win; repeating on a `CANCELLED`/`COMPLETED` session → `409 SESSION_NOT_ACTIVE` with no state change. The released slot is bookable again immediately.
>
> **Phase 9, Block 5 — operator occupancy reporting:** `GET /operators/me/reports/occupancy` is **implemented** (mounted at `/api/v1/operators`). Requires a **VERIFIED** operator org (registered-but-incomplete → `404 OPERATOR_NOT_FOUND`, PENDING/UNDER_REVIEW/REJECTED → `403 OPERATOR_NOT_VERIFIED`; plain user → `403 FORBIDDEN`). It is a read-only, server-scoped aggregate — the caller's facilities are derived from `parking_facilities.operator_id` and the query never carries a facility/operator id (no IDOR surface, `docs/SECURITY.md` §5). Query params `from`/`to` are optional ISO-8601 instants (`400 VALIDATION_ERROR` on malformed or `from > to`); when omitted the bounds echo the observed session range (min `entry_at` … max `exit_at`); when provided they are echoed back verbatim. Response `{ start, end, totalFacilities, totalSlots, availableSlots, occupiedSlots, activeSessions, completedSessions, cancelledSessions }`: slot + active counts are **live** (from `parking_slots.status` / `parking_sessions.status`), while `completedSessions`/`cancelledSessions` are counted by `exit_at` **within the period** (a session active at read time is not a completion/cancellation; sessions ended outside the filter window are excluded). Booked-but-not-entered reservations leave occupancy unchanged. Soft-deleted facilities and their slots/sessions are excluded.
...
| GET | /operators/me/reports/occupancy | operator (verified) | basic occupancy report (live + period) |
...
> **Phase 9, Block 3 — this legacy contract is superseded.** The implemented gate flow is the **parking-pass token**: `GET /parking-sessions/by-reservation/{code}/pass` returns a deterministic `ppk_` JWT, and `POST /parking-sessions/entry { verificationToken }` verifies it (digest vs `reservations.verification_token_hash`, deterministic → no turnover/rotation, `exp` bound to the reservation window) and opens the session atomically. The `/gate/tokens/verify` shape above stays as the reference for a future independent gate/ANPR reader contract.
>
> **Session 12 — driver QR + operator gate scanning (frontend only, no new endpoint):** the driver's pass card renders the **same** `ppk_` string as a QR code (`qrcode` lib) and keeps the copy/hide fallback, so a gate operator can scan it; the operator dashboard's parking-entry form (`OperatorDashboard.tsx`) accepts a scanned value via the `html5-qrcode` camera scanner (started only on explicit "Scan QR code", cleaned up on success/cancel/unmount) and submits it through the **existing** `enterParking({ verificationToken })` flow (no new backend endpoint, error vocabulary unchanged). The QR payload is the raw token only — no PII, reservation ids, or secrets; the token is never logged.
