...
> - **Phase 9, Block 5 — occupancy reporting:** `GET /operators/me/reports/occupancy` is an operator-scoped **read-only aggregate** — the report's scope is derived entirely from the authenticated org (`parking_facilities.operator_id`); the query accepts only period filters and never a facility/operator id, so there is no ownership parameter to tamper with (no IDOR surface). Access requires a **VERIFIED** operator org; registered-but-incomplete → `404 OPERATOR_NOT_FOUND`, non-verified → `403 OPERATOR_NOT_VERIFIED`, plain users → `403 FORBIDDEN` (mirrors `GET /operators/me/sessions`). Period bounds are validated (`400 VALIDATION_ERROR` on malformed/`from > to`). No new audit actions were introduced; the report reads the same transactions the session lifecycle already audits.
- **Gate verification (Block 3):** `POST /parking-sessions/entry` accepts exactly one of `reservationCode`/`verificationToken` (zod `.strict()`, bounds ≤64 / ≤4096) and verifies tokens by digest + reservation window (`TOKEN_NOT_YET_VALID`/`TOKEN_EXPIRED`) — no online JWT secret validation of user-supplied values, no timing side channel beyond the sha256 compare. Rejections persist `GATE_ENTRY_REJECTED`/`GATE_EXIT_REJECTED` with `metadata.reason` = the error code (no token material); successes emit the standard transaction audits `PARKING_SESSION_ENTRY`/`PARKING_SESSION_EXIT` (the `GATE_ENTRY_VERIFIED`/`GATE_EXIT_VERIFIED`/`SLOT_OCCUPIED`/`SLOT_RELEASED` verbs are reserved vocabulary only, not raised — docs corrected in Session 12). The slot-occupancy manual guard (`SLOT_IN_USE`) prevents hand-flipping a slot that backs an active session, closing the double-occupancy CLI/operator path.
