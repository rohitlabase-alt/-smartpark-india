...
## D-035 — Phase 7 mock payment foundation (PENDING_PAYMENT lifecycle; hours-based JSONB pricing; no refunds)

- **Decision:** Add the **mock payment** flow on top of the Phase 2C booking foundation. Migration `0006` activates the payment fields of `reservations` (`DATABASE.md` §2.12): the `state` CHECK now covers the full vocabulary `('PENDING_PAYMENT','CONFIRMED','ACTIVE','COMPLETED','CANCELLED','EXPIRED','FAILED')`, `amount`/`payment_status` are used, and the **live exclusion predicate is widened to `state IN ('PENDING_PAYMENT','CONFIRMED','ACTIVE')`** so an unpaid pending reservation still holds its slot (transition to `CONFIRMED` on the same slot/window does not self-conflict). New tables: `payments` (§2.15), `transactions` (§2.16), `payment_idempotency_keys` (§2.17).
  - **Reservation amounts** come from the facility's `parking_facilities.pricing` JSONB (`D-001`/`D-011` allow referencing the pricing snapshot) using a **`{ "hourlyRate": <positive number> }`** contract; `readHourlyRate` falls back to `DEFAULT_MOCK_HOURLY_RATE = 100` INR when absent/invalid. `amount = round(ceil(hours) × rate, 2)` with `hours = max(1, ceil((ends − starts)/3600000))` (rounded per D-008).
  - **Lifecycle:** create → `PENDING_PAYMENT` (`amount` + `payment_status='INITIATED'`). `POST /api/v1/payments/initiate` creates a `payments` row (provider `MOCK`, deterministic `providerTxnId` = `MOCK-<code>-<amount>`) and, when an `Idempotency-Key` is supplied, atomically claims a `payment_idempotency_keys` row (15 min TTL) so repeats reuse the existing payment. `POST /api/v1/payments/:txnId/verify` runs in **one DB transaction**: `MockPaymentProvider.verify` returns FAILED when the txn id ends in `__FAIL__` (test-only injection or a directly inserted row), otherwise SUCCESS. SUCCESS → payment `SUCCESS` + reservation `CONFIRMED` (`confirmed_at`, `payment_status='SUCCESS'`) + one `CHARGE`/`SUCCESS` transaction. FAILED → payment `FAILED` + reservation `FAILED` (never CONFIRMED) + one `CHARGE`/`FAILED` transaction. Re-verifying SUCCESS is idempotent; re-verifying FAILED → `409 PAYMENT_ALREADY_FAILED`; verifying when the reservation is no longer pending → `409 RESERVATION_NOT_CONFIRMABLE` (whole verification rolls back). Ownership is enforced server-side (`paymentsFindOwnedByProviderTxnId` joins `reservations.user_id`); other users → `404 PAYMENT_NOT_FOUND` (no IDOR enumeration).
  - **Cancellation:** customer `cancelBooking` permits `PENDING_PAYMENT` + `CONFIRMED` (still `409 CANNOT_CANCEL` otherwise, `422 CANNOT_CANCEL_COMPLETED` on completed, `409 ALREADY_CANCELLED` on repeat). **No refund workflow** in the mock: cancellation does not reverse the `CHARGE` or set the payment to `REFUNDED` (`REFUND`/`REVERSAL` transaction kinds are permitted by the schema but unused; refunds land with a real provider).
- **Deferred (tokens phase):** `POST /reservations/{code}/confirm`, parking tokens/QR codes, gate entry, `ACTIVE`/`EXPIRED` entry (states exist in the CHECK but nothing transitions to them yet), a real (non-mock) payment provider.
- **Rationale:** reservation state must model "held but unpaid" before any token flow; the DB-level exclusion constraint (D-007/D-034) stays the race-safe double-booking guard (now crediting `PENDING_PAYMENT`); the deterministic mock provider needs no secrets/cards/external API and gives testable SUCCESS/FAILED outcomes; idempotency (API_SPEC §6) prevents duplicate attempts without a distributed lock; server-side ownership + 404 keeps the IDOR requirement (§5); hours-based JSONB pricing avoids a `pricing_rules` table until a real pricing engine lands.
- **Status:** ACTIVE — verified end-to-end (migration 0006 applied + idempotent; 23 new DB-backed payment tests; all backend suites green; frontend typecheck/tests green after the shared `Reservation.amount`/`paymentStatus` addition).

---

## D-037 — Phase 9 parking sessions: single-active-per-reservation, hash-at-rest entry tokens

- **Decision:** Implement the parking session entry/exit foundation as a new `parking_sessions` table (migration `0009`, DATABASE §2.13) plus a `sessions` module under `/api/v1/parking-sessions`. A parking session is a **ledger entry** created when a paid `CONFIRMED` reservation actually enters the facility and completed when it exits; the **slot's operational status stays the single source of truth for occupancy** (`parking_slots.status`, `availability_state` mirrored), so no second, conflicting occupancy source is introduced.
- **Single active session per reservation (not 1-1):** uniqueness is enforced by **partial unique indexes** — `(reservation_id) WHERE status = 'ACTIVE'` and `(slot_id) WHERE status = 'ACTIVE'` — so the same reservation can be entered repeatedly **over time** (status `'CANCELLED'` reserves an operator-cancellation path), but there is never more than one active session at once. This supersedes the earlier §2.13 draft's hard 1-1 `reservation_id UNIQUE` and is immune to double-entry at the DB level in addition to the reservation-state guards.
- **Entry token: one-time bearer credential, stored hashed.** `POST /parking-sessions/entry` mints a high-entropy `ses_<48 hex>` token and returns it **exactly once** (safe to embed in a future QR/gate flow); the database stores **only its SHA-256 digest** (`entry_token_hash`), matching the refresh-token at-rest convention (D-030). The digest is never served back, the raw token never appears in audit metadata (sanitizer drops token-looking keys), and concurrent entry requests serialize on a `FOR UPDATE` reservation row plus a guarded slot update.
- **Authorization:** a session may be created/read/completed by the **reservation owner** or the **VERIFIED operator who owns the facility** — enforced in SQL (ownership join → reservation → facility → operator), with any miss surfacing as `404` (no existence disclosure). The operator path requires a verified org; unverified operators resolve to `null` and can only act on their own reservations.
- **State machines (atomic, audited):** entry — reservation `CONFIRMED → ACTIVE`, slot `→ OCCUPIED`, cache `→ OCCUPIED`, audit `PARKING_SESSION_ENTRY`; exit — session `ACTIVE → COMPLETED`, reservation `ACTIVE → COMPLETED`, slot `→ AVAILABLE` (operator-set statuses preserved; release guarded to `OCCUPIED`), audit `PARKING_SESSION_EXIT`. Entry requires facility `VERIFIED` + active and a reservation with an assigned slot.
- **Deferred to Phase 9 Block 2+:** QR/gate verification endpoint, time-window enforcement (`[starts_at, ends_at)`), cancellation/`CANCELLED` transition, operator session lists/reports, IoT integration.
- **Status:** ACTIVE — verified end-to-end (new DB-backed `parking-sessions.integration.test.ts`; frontend `api/sessions.ts` client).
...
- **Audit:** rejections write `GATE_ENTRY_REJECTED`/`GATE_EXIT_REJECTED` with the mapped error code as `reason`; successful entry/exit keep the Block 1 transaction audits `PARKING_SESSION_ENTRY`/`PARKING_SESSION_EXIT` (the `GATE_ENTRY_VERIFIED`/`GATE_EXIT_VERIFIED`/`SLOT_OCCUPIED`/`SLOT_RELEASED` verbs are reserved vocabulary only, not raised — docs corrected in Session 12). Raw tokens never appear in audit metadata, logs, or responses beyond the single issuance/entry call.
...
## D-040 — Phase 9 Block A: QR transport for the parking pass + operator gate scanning (frontend only)

- **Decision:** Present the **existing deterministic `ppk_` token as the QR payload** and let gate operators scan it — **no second token system, no new backend endpoint, no change to pass-token crypto.** Driver `ParkingSessionPanel` renders `GET /parking-sessions/by-reservation/{code}/pass`'s already-issued `verificationToken` via `qrcode` (only after an explicit "Show parking pass" — the pass fetch stays lazy and is never triggered on page load; Copy/Hide preserved); the operator entry form scans via `html5-qrcode` camera and submits the decoded string through the **existing** `enterParking({ verificationToken })` flow, so authorization, window checks, digest-at-rest, occupancy guards, and the error vocabulary are all reused unchanged. The QR payload is the raw token string only — never PII, DB ids, facility ids, or secrets.
- **Libs (minimal, frontend-only):** `qrcode@1.5.4` (+ `@types/qrcode` as devDep) for rendering via `toDataURL`; `html5-qrcode@2.3.8` (self-typed, ESM) for camera scanning bound to `{ facingMode: "environment" }`. Both are small, maintained, browser-compatible; no new backend deps.
- **Gate-staff RBAC is out of scope.** `GATE_STAFF` remains defined-but-unenforced shared vocabulary (no `requireRole("GATE_STAFF")`, no `/gate/*` routes); scanning runs under the existing `PARKING_OPERATOR` + VERIFIED-facility scoping (`enterParking` resolves facility ownership server-side; a body `facilityId` is never trusted).
- **Camera discipline:** permission is only requested after an explicit "Scan QR code" click; the `Html5Qrcode` instance is always disposed (`stop()` + `clear()`) on success, cancel, or dashboard unmount (request-id guarded); a second scan result while one is in flight is ignored (exactly one entry POST).
- **Status:** ACTIVE — verified end-to-end (web tests in `OperatorDashboard.test.tsx` + `ParkingSessions.test.tsx`; see CHANGELOG Session 12).

## D-041 — Phase 9 Block 5 operator occupancy reporting (server-scoped, live + period, lazy UI)

- **Decision:** Ship a **read-only, server-scoped** occupancy aggregate `GET /api/v1/operators/me/reports/occupancy` (+ a "View occupancy report" panel in the operator dashboard) as the first reporting capability; it is the foundation the IoT track will later feed (`ROADMAP.md` PHASE 10). No separate "</reporting>" tables, materialized views, or pipeline: one idempotent SELECT of scalar aggregates over the existing `parking_facilities` / `parking_slots` / `parking_sessions` tables (`deleted_at IS NULL` joins), mirroring the `platform.repository.ts` pattern.
- **Scope is derived, never client-supplied.** The caller's facilities come from `parking_facilities.operator_id` for the authenticated org — the query exposes only optional period filters (`from`/`to` ISO-8601, `400 VALIDATION_ERROR` on malformed or `from > to`). There is no facility/operator id anywhere in the request, so there is no IDOR parameter to tamper with (no exception: registered-but-incomplete org → `404 OPERATOR_NOT_FOUND`, PENDING/UNDER_REVIEW/REJECTED → `403 OPERATOR_NOT_VERIFIED`, plain user → `403 FORBIDDEN`).
- **Count semantics (contract):** slot and active counts are **live** — `availableSlots`/`occupiedSlots` from `parking_slots.status`, `activeSessions` from current `ACTIVE` sessions (a blocked-but-not-entered reservation changes nothing, since creating a reservation never flips slot status). `completedSessions`/`cancelledSessions` are counted by `exit_at` **within the requested period** — `exit_at` is the canonical session-end timestamp for both `COMPLETED` (Block 1) and `CANCELLED` (Block 4.2). `start`/`end` echo the `from`/`to` filters verbatim, falling back to the observed min `entry_at` … max `exit_at` (null when no sessions). A session active at read time counts toward `activeSessions` regardless of the period; only completions/cancellations are period-bounded.
- **UI is lazy.** The dashboard's mount chain is a strict, test-asserted 5-fetch sequence (operator → facilities → reservations → sessions → slots, the latter via the auto-selected first facility). Fetching the report on mount would break that invariant and pay for data the operator didn't ask to see — so the report loads **only on an explicit "View occupancy report" click** (subsequent "Refresh report" re-fetches). Identical "don't fetch unnecessarily" discipline to the driver parking-pass reveal (D-040/D-038).
- **Status:** ACTIVE — verified end-to-end (20 DB-backed tests in `operators-reports.integration.test.ts`; 6 API-client tests + 4 dashboard tests in web). CHANGELOG Session 13.
...
|---|---|---|---|---|---|
| 2026-09-16 | D-041 | Added — Phase 9 Block 5 operator occupancy reporting (`GET /operators/me/reports/occupancy`; server-scoped by `parking_facilities.operator_id`, never client-supplied; live slot/active counts + period-bounded completed/cancelled by `exit_at`; `start`/`end` echo filters or observed bounds; lazy "View occupancy report" UI — no mount fetch) | Phase 9 occupancy reports (foundation for IoT track) | operators (reports), shared, web, docs | none (no migration) |
| 2026-09-16 | D-040 | Added — Phase 9 Block A QR pass transport + operator gate scan (QR payload = existing `ppk_` token string via `qrcode`; `html5-qrcode` camera scan → existing `enterParking({verificationToken})`; lazy reveal kept, camera only on explicit click, scanner disposed on success/cancel/unmount; no new endpoint/token; GATE_STAFF unenforced RBAC out of scope) | Phase 9 gate QR integration | web, shared (no change), docs | none (no migration) |
