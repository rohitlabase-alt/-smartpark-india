...
export const facilitiesService = {
  async createFacility(userId: number, input: CreateFacilityRequest): Promise<ParkingFacility> {
    ...
    return withTransaction(async (client) => {
      const sequence = await facilitiesRepository.nextParkingSequence(client);
      ...
      await auditService.createEvent(client, {
        ...
        metadata: { facilityName: facility.name, city: facility.city },
      });
  ...
  async updateFacility(
    ...
    return withTransaction(async (client) => {
      ...
      const updated = await facilitiesRepository.update(
        facilityId,
        {
          name: input.name?.trim(),
